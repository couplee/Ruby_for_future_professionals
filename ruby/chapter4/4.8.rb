...
a.delete 100 do
  ...
  このコードは動く。この状態からdo...endを{}に置き換えて実行する
a.delete 100 { 'NG' }  #=> Syntax Error

  このようにエラーになってしまう理由は、{}の結合度が強く、a.delete 100ではなくて、100 { 'NG' }と解釈されてしまうから。
  だが、100はただの数値であって、メソッドではないからブロックを渡すことができず、構文エラーと解釈されてしまうわけ。
  このエラーを解決するためには、100を()で囲む。こうしたら、a.delete(100)だと優先的に解釈されるようになる。
a.delete(100) { 'NG' }  #=> "NG"

  このように引数付きのメソッド呼び出しで{}をブロックとして使う場合は、メソッド引数の()を省略できないと覚える



                            💠 4.8.8 ブロックを使うメソッドを定義する
  ブロックを使うメソッドは自分で定義することができる。(10章で詳しく学習する)
  ex) 名前の配列をmapメソッドで全部「さん付け」にして、最後に「AさんとBさん」みたいに、「と」を使って連結する
names = ['田中', '鈴木', '佐藤']
san_names = names.map { |name| "#{name}さん" }  #=> ['田中さん', "鈴木さん", "佐藤さん"]

  上のコードはmapメソッドの戻り値をsan_namesという変数に格納してるけど、下のコードみたいにブロックの後ろにドット(.)を書いたら、
  mapメソッドの戻り値に対して、直接joinメソッドを呼び出すことができる
names = ['田中', '鈴木', '佐藤']
names.map { |name| "#{name}さん" }.join('と')  #=> "田中さんと鈴木さんと佐藤さん"

　また、{}とdo...endは基本的に同じだから、endの後ろにドットをつけてメソッドを呼び出すこともできる
names = ['田中', '鈴木', '佐藤']
names.map do |name|
  "#{name}さん"
end.join('と')  #=> "田中さんと鈴木さんと佐藤さん"

  だけどブロックのうしろに別のメソッドを続けて書く場合、do...endよりも{}を使った方が読みやすいという意見もあるから、
  どちらかというと{}を使った方が良いかもしれない。
  このように、変数を使わんとメソッドの戻り値に対して、直接ほかのメソッドを呼び出していくコーティングスタイルのことをメソッドチェーンと呼ぶ。
  メソッドチェーンについては第10章のコラム「メソッドチェーンを使ってコードを書く(P.387)」で再度学習する


  配列をもっと上手に使いこなすために(P.132参照)



              💠 4.9 さまざまな繰り返し処理
  配列とeachメソッドの組み合わせはRubyにおける繰り返し処理の代表例だが、Rubyには繰り返し処理を行う方法が他にもいっぱいある
  
  
  
                💠 4.9.1 timesメソッド
  配列を使わず、単純にn回処理を繰り返したい、というときはIntegerクラスのtimesメソッドを使うと便利
sum = 0
5.times { |n| sum += n}  # 処理を5回繰り返す。nには0, 1, 2, 3が入る
sum  #=> 10

  不要だったらブロック引数は省略しても良い
sum = 0
5.times { sum += 1 }  # sumに1を加算する処理を5回繰り返す
sum  #=> 5



          💠 4.9.2 uptoメソッドとdowntoメソッド
  nからmまで数値を1つずつ増やしながら何かを処理したい場合、Integerクラスのuptoメソッドを使う
a = []
10.upto(14) { |n| a << n }
a  #=> [10, 11, 12, 13, 14]

  逆に数値を減らしていきたい場合、downtoメソッドを使う
a = []
14.downto(10) { |n| a << n }
a  #=> [14, 13, 12, 11, 10]
              ...
              💠 4.9.3 stepメソッド
  1, 3, 5, 7みたいに、nからmまで数値をx個ずつ増やしながら何かを処理したい場合、Numericクラスのstepメソッドを使う。stepメソッドの構文は下記参照
開始値.step(上限値, 1度に増減する大きさ)

  ex) 1から10まで2つずつ値を増やしながら処理をしたい場合
a = []
1.step(10, 2) { |n| a << n }
a  #=> [1, 3, 5, 7, 9]

  ex) 10から1まで2つずつ値を減らす場合
a = []
10.step(1, -2) { |n| a << n }
a  #=> [10, 8, 6, 4, 2]
            ...
            💠 4.9.4 while文とuntil文
  ここまではオブジェクトのメソッドとブロックを組み合わせて繰り返し処理を実行してきたが、Rubyには繰り返し処理用の構文も用意されてる。
  その1つがwhile文。while文は指定した条件が真である間、処理を繰り返す。
while 条件式（真であれば実行）
  繰り返したい処理
end
...
a  #=> [1, 1, 1, 1, 1]
  ...
  だけど、1行で書く場合は修飾子としてwhile文を後ろに置いた方がスッキリする
a = []
a << 1 while a.size < 5
a  #=> [1, 1, 1, 1, 1]

  どんな条件でも最低1回は実行したい場合、begin ... endで囲んでからwhileを書く
a = []

while false
  a << 1  # このコードは常に条件が偽になるから実行されない
end
a  #=> []

begin  # begin ... endで囲むとどんな条件でも最低1回は実行される
  a << 1
end while false
a  #=> [1]
...
sum_value  #=> 40
  ...
  このような微妙な違いはあるが、Rubyではfor文ではなくeachメソッドやmapメソッドといった繰り返し処理用のメソッドを使う場合がほとんどである
      ...
      💠 4.9.6 loopメソッド
  あえて無限ループを作りたいというときは、while文を使って下記のようなコードが書ける
while true
  # 無限ループ用の処理
end

  上記のコードに加えてもう一つ、Kernelモジュールのloopメソッドとブロックを使う方法がある。
  Kernelモジュールについては第8章で学習するから、ここではputsメソッドやpメソッドのようにloopメソッドも「どこでも呼べるメソッド」
  と考えればいい。下記はloopメソッドの構文
loop do
  # 無限ループ用の処理
end

  無限ループから脱出するにはbreakを使う。
    ex) 配列に格納した5つの数値の中からタンダムに数値を選んで、5が出たタイミングで脱出する無限ループのサンプルコード
numbers = [1, 2, 3, 4, 5]
loop do
  n = numbers.sample  # sampleメソッドでランダムに要素を一つ取得する
  puts n
  break if n == 5
end
#=> 3
...
#   5
  ...
  loopメソッドはブロックを使うから変数nがループの外で参照できない。一方でwhile文はループの外でもnを参照できる。
  この違いはfor文とeachメソッドの違いと同じだから、詳しい理由はfor文の項を参照する



          💠 4.10 繰り返し処理用の制御構造
  「4.9.6 loopメソッド」の値ではループを脱出するためにbreakを使った。Rubyには下記のように他にも繰り返し処理の動きを変えるための制御構造が用意されてる。
    ・break
    ・next
    ・redo
      
  また、Kernelモジュールのthrowメソッドとcatchメソッドもbreakと同じような用途で使われる。下記の値でそれぞれについて学習する
